...
A 'dmesg -c >/dev/null; (printf "start\n" | timeout 8 /vendor/bin/spectraltool -i wlan0 >/data/local/tmp/sps.out 2>&1 &); sleep 9; echo ---TOOL---; cat /data/local/tmp/sps.out 2>/dev/null; echo ---DMESG---; dmesg | grep -iE "spectral|null_|scan|phyerr|report" | tail -25'